...
Plenty of portfolio projects show you can build on top of a distributed system. Very few show the layer underneath — how a handful of machines agree on a single source of truth when one of them crashes, or when the network splits them in half and both halves think they're in charge.
...
The bar I set for myself: **every safety property in the paper should have a test that fails if I break it.** Not a test that passes because the happy path happens to work — a test that would go red if I deleted the rule it's checking. More on that below, because it turned out to be harder than it sounds.

---

## What works right now

**Leader election** — randomized timeouts, one vote per node per term, the §5.4.1 restriction that stops a node with a stale log from ever winning. Split votes resolve instead of deadlocking.

**Log replication** — AppendEntries with the log matching property, conflict repair that backs up a whole term per round trip rather than one entry at a time, and commit advancement that follows §5.4.2 properly (more on this in a second, because it's the rule most toy implementations get wrong).
...
**Linearizable reads** — the read-index protocol. A leader confirms with a majority that it's *still* the leader before answering, so a partitioned-off leader can't serve you stale data while believing everything is fine.
...
Not "minimal" — none. `internal/raft` doesn't import `time`, doesn't start a goroutine, and never touches a socket. A node advances when you call `Tick()`, receives a message when you call `Step(msg)`, and hands you everything it wants to do — messages to send, entries to apply — from `Ready()`.

This sounds like an inconvenience and it is the single best decision in the project. It means an entire five-node cluster runs inside one goroutine with a fake clock, and a test that fails does so *identically* every time. No sleeps, no polling, no "run it again and see." When I get to the chaos harness in Phase 5 — partitions, message reordering, crashes mid-write — this is what makes those scenarios reproducible instead of a flaky mess I'd learn to ignore.
...
Raft says an entry is committed once a majority stores it. That's *not sufficient*, and §5.4.2 of the paper exists to explain why. An entry from a previous term can sit on a majority of nodes and **still get overwritten** by a future leader. If you commit it on replica count alone, two state machines diverge and nothing anywhere reports a problem. It's Figure 8 in the paper, and it's the difference between a Raft implementation and something that looks like one.

The entry only becomes safe once something from the leader's *own* term commits on top of it. That's why every new leader appends a no-op the moment it's elected — it gives it an in-term entry to commit immediately, which transitively secures everything inherited.

I wrote a test for this. **It passed, and it was worthless.** It only checked that a leader commits its own-term entry — it never built the dangerous case at all. A green test on the most important rule in the codebase, verifying nothing. I threw it out and wrote one that constructs the Figure 8 setup directly, then deleted the guard to confirm the test actually goes red. It does, and it's the only test that does.
...
**A single-node cluster could never commit anything.** `becomeLeader` appends its no-op and broadcasts, but with no peers there's nothing to broadcast and no response ever arrives to advance the commit index. So the node became leader and then sat there, unable to commit its own entry. Present since Phase 1, invisible because the test only checked that it became leader.

**On restart with a compacted log, the node panicked.** The log's committed/applied cursors started at zero, but the storage had been compacted past that point — so the first `Ready()` went looking for entries that no longer existed. What makes this one interesting: **neither package's tests could have found it.** The Raft tests use in-memory storage that's never compacted; the storage tests never run the consensus core. It took both layers running together.

**Compaction could delete your vote.** Hard state records live in WAL segments interleaved with log entries, so deleting an old segment could take the most recent vote with it — and a node that forgets its vote can vote twice in one term and elect two leaders. Found while writing the WAL, not by a test, which is its own kind of luck.
...
| Property | Test |
|---|---|
| Election Safety | asserted continuously by the harness — *every* test checks it |
...
**Hand-rolled disk format, not protobuf or gob.** The Raft log is the part whose on-disk representation I should be able to explain byte by byte. A fixed little-endian layout is also debuggable with a hex dump when a record goes bad. (The KV *values* are opaque bytes — different call, made deliberately.)

**Read-index rather than leader leases.** Leases are faster — no round trip — but they buy that by assuming clocks don't drift more than some bound. Read-index costs one round trip and assumes nothing about clocks. For a project about correctness under adversarial conditions, trading a network assumption for a timing assumption is the wrong direction. The tradeoff is written up in `readonly.go`.

**Deduplication lives in the state machine, not the server.** A server-side check only filters duplicates arriving at the node that saw the original — the entry still commits and applies everywhere else, and the replicas **diverge**. That's strictly worse than a duplicate.

**Sorted keys in snapshots.** Go randomizes map iteration, so an unsorted encoding produces different bytes every time for identical state. Sorting makes a snapshot a deterministic function of the state, which makes two replicas' snapshots directly comparable — the cheapest possible convergence check, and something the chaos harness will lean on.
...
- **No chaos harness yet.** This is Phase 5 and it's the centerpiece of the whole project — fault injection, a linearizability checker, a documented set of adversarial scenarios with pass/fail results. The deterministic core was built specifically to make it possible. It doesn't exist yet.
...
- **No cluster membership changes.** Phase 4. Membership is fixed at startup.
